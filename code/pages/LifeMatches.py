...
def make_predict_upd(rad_team, dire_team):
    if rad_team not in teams_dict or dire_team not in teams_dict:
        return 'Нет прогноза', 'Нет прогноза'
...
if r_steam.status_code != 200:
    ...
    if len(set(important_cols) - set(live_df.columns)) != 0:
        ...
        if len(filtered_df) == 0:
            ...
            filtered_df = filtered_df[filtered_df['winner_side'] != 'Нет прогноза'].reset_index(drop=True)
        ...
        st.markdown('- Получаю из Steam API онлайн идущие профессиональные матчи игры дота 2')
        st.markdown('- Делаю для них предсказание победителя с помощью предобученой модели')
        st.markdown('- Публикую прогноз в свой телеграм канал')